...
Future<void> _firebaseMessagingBackgroundHandler(RemoteMessage message) async {
  ...
  final wantsConnected =
      message.data['route'] == 'connected' ||
      message.data['action'] == 'connected';
  ...
  if (wantsConnected) {
    ...
    final isStream =
        message.data['action'] == 'stream_request' ||
        message.data['route'] == 'stream';
    await _showNotification(
...
class MyApp extends StatefulWidget {
  ...
  static bool wasBackgroundLaunch =
      false; // Track if notification came from background
  ...
  void initState() {
    ...
    FirebaseMessaging.onMessage.listen((RemoteMessage message) {
      ...
      if (wantsConnected) {
        ...
        _showNotification(
          message.data['title'] ??
              (isStream ? 'Stream Request' : 'Notification'),
          message.data['body'] ??
              (isStream ? 'Tap to view stream' : 'Tap to open'),
          payload: isStream ? 'route=stream' : null,
        );
      }
    });
